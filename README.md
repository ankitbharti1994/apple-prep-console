...
npm run preview    # serve the built site (this is the one with search working)
...
`npm run dev` has no search index — Pagefind is generated by `npm run build`, so use
`npm run preview` to try ⌘K full-text search.

> **Serving `dist/` yourself?** Use a plain static server, not SPA mode.
> `npx serve -s dist` and `python -m http.server` with a catch-all rewrite will
> serve `index.html` for *every* route, so every tab shows the overview. This is a
> multi-page site: `/plan` must resolve to `dist/plan/index.html`.
> Plain `npx serve dist` is fine.
